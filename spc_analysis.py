...
def get_rmsd(
    ...
    assert (
        isinstance(ref, mda.Universe) or ref == None
    ), "ref should be a MDAnlaysis universe or None."
    assert isinstance(sel_str, str), "sel_str should be string."
    assert (
        isinstance(sel_str_al, str) or sel_str_al == None
    ), "sel_str_al should be string."
    ...
    assert (
        isinstance(start, int) or start == None
    ), "start should be int or None."
    ...
    if write_trj == True:
        rmsd = (
            AlignTraj(u, ref, select=sel_str_al, filename=trj_path)
        ...
        rmsd = (
            AlignTraj(u, ref, select=sel_str_al)
            .run(start=start, stop=stop, step=step, verbose=verbose)
            .rmsd
        )
        u.trajectory[0]
    ...
    for i, ts in tqdm(
        enumerate(u.trajectory[start:stop:step]),
        total=u.trajectory.n_frames / step - 1,
        disable=not verbose,
    ):
        t.append(ts.dt * i)
...
def distance_atom_groups(
    ...
    Parameters
    ----------
    u: MDA universe to analyze trajectory to analyze.
