...
def format_plus_minus_error(
    value, error, format_value=".1f", format_error=".1f"
):
    """
    ...
    assert isinstance(value, Iterable) and all(
        isinstance(x, float) for x in value
    ), "Value is not an iterable of floats."
    assert isinstance(error, Iterable) and all(
        isinstance(x, float) for x in error
    ), "Error is not an iterable of floats."
    ...
    with open(path) as file:
        for line in file:
            if line[0] != "#":
                ...
                if ((x_lims[0] > int(l[1])) and (x_lims[0] > int(l[2]))) or (
                    (x_lims[1] < int(l[1])) and (x_lims[1] < int(l[2]))
                ):
                    continue
                else:
                    ...
                    if l[0] == "H":
                        ...
                        rect_backgr = mpatches.FancyArrowPatch(
                            (x_tail, y0),
                            (x_head, y0),
                            arrowstyle=rect_style,
                            color="white",
                            zorder=0,
                            transform=trans,
                            shrinkA=0,
                        ...
                        arrow_style = (
                            "simple,head_length=15,head_width=30,tail_width=10"
                        )
                        ...
                        ax.text(
                            ...
                            transform=trans,
                            bbox=dict(
                                facecolor="white", alpha=0, edgecolor="White"
                            ),
                            color=SF_color,
                        )
...
def my_plot_ppc(trace, label_array, enc_dic=None, **kwargs):
    ...
    from arviz import InferenceData, plot_ppc
    from matplotlib.pyplot import subplots
    from numpy import array, unique

    assert (
        isinstance(label_array, Iterable)
        and all(isinstance(x, int) for x in label_array)
        and all(label_array >= 0)
    ), "label_array is not an iterable of non-negative ints."
    assert isinstance(
        trace, InferenceData
    ), "trace is not an arviz inference object"
    key = list(trace.posterior_predictive.data_vars.keys())
    assert (
        len(key) == 1
    ), "There should be only one data_var in trace.posterior_predicitve"
    ...
    key = key[0]
    assert (
        isinstance(enc_dic, dict) or enc_dic == None
    ), "enc_dic should be a dictionary"

    allowed_plot_kwargs = ["alpha"]
    plot_kwargs = {
        k: kwargs.pop(k)
        for k, v in list(kwargs.items())
        if k in allowed_plot_kwargs
    }
    ...
    if enc_dic == None:
        ...
        assert all(isinstance(x, int) for x in enc_dic.keys()) and all(
            array(list(enc_dic.keys())) >= 0
        ), "enc_dic is not made of non-negatives."
        assert (
            len(list(enc_dic.keys())) >= n_plots
        ), "enc_dic does not have enough keys"

    fig, ax = subplots(
        n_rows,
        n_cols,
        figsize=(sizex, 5.5 * n_rows),
        sharex=False,
        sharey=False,
    )
    ax = ax.flatten()
    for i in range(n_plots):
        trace.observed_data["ppc"] = trace.observed_data[key][
            array(label_array) == i
        ]
        trace.posterior_predictive["ppc"] = trace.posterior_predictive[key][
            :, :, :, i
        ]
